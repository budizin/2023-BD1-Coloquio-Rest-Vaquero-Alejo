...
class Orderdetails(models.Model):
    orderid = models.ForeignKey('Orders', models.CASCADE, db_column='OrderID')  # Field name made lowercase. The composite primary key (OrderID, ProductID) found, that is not supported. The first column is selected.
...
class Products(models.Model):
    ...
    def calcularDiferenciaDeStock(self,stockRequerido):
        return stockRequerido - (self.unitsinstock + self.unitsinstock) 
    def __str__(self):
        return f"{self.productid} - {self.productname} - {self.supplierid} - {self.unitsonorder + self.unitsinstock}"
